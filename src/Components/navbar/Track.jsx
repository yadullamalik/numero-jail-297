import React from 'react'
import "./Track.css"

const Track = () => {
  return (
    <div className='track-main'>
        <p className='track-p'> TIME TRACKING FOR EVERY TEAM SIZE</p>

        <div className='track-main2'>
        <div className='track-sub1'>
            <div><img className='track-img1' src="data:image/svg+xml;base64," alt="" /></div>
            <div>
                <p className='track-h1'> Key Features </p>
                <p className='track-h2'> View features by need, from time tracking to project annd team management</p>
            </div>
        </div>
        <div className='track-sub2'>
            <div><img className='track-img1' src="data:image/svg+xml;base64," alt="" /></div>
            <div>
                <p className='track-h1' >Employee time tracking </p>
                <p className='track-h2'>Loved by teams and team leads. Error-free timesheet data and ease of use</p>
            </div>
        </div>
        </div>

    </div>
   
  )
}

export default Track

